<aside class="sticky top-0 mx-6 px-4 pt-16 space-y-4 h-screen bg-gray-50">
    @if($exam !== null)
        <div class="w-full p-4 bg-black rounded-xl shadow-lg shadow-slate-200">
            <p class="text-sm text-white">{{ $exam->name }}</p>
            <p class="text-sm text-white">{{ date('j F Y', strtotime($exam->exam_date)) }}</p>
            <div class="flex flex-wrap mt-2 items-center font-bold">
                <p class="text-white truncate mr-2 max-w-xs">{{ $exam->course()->name }}</p>
                <p class="text-gray-300">({{ $exam->course()->code }})</p>
            </div>
        </div>

        <form method="POST" action="{{ route('enroll.exams.enroll') }}">
